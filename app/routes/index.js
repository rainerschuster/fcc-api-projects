...
/* 01 TIMESTAMP BEGIN */
    app.route('/timestamp')
		.get(isLoggedIn, function (req, res) {
			res.sendFile(path + '/public/timestamp.index.html');
		});
...
/* 02 WHOAMI BEGIN */
    app.route('/header')
		.get(isLoggedIn, function (req, res) {
			res.sendFile(path + '/public/header.index.html');
		});
...
/* 03 URL BEGIN */
    app.route('/urlshortener')
		.get(isLoggedIn, function (req, res) {
			res.sendFile(path + '/public/urlshortener.index.html');
		});
	app.route('/api/new/*?')
...
/* 04 IMAGESEARCH BEGIN */
    app.route('/imagesearch')
		.get(isLoggedIn, function (req, res) {
			res.sendFile(path + '/public/imagesearch.index.html');
		});
...
/* 05 METADATA BEGIN */
    app.route('/metadata')
		.get(isLoggedIn, function (req, res) {
			res.sendFile(path + '/public/metadata.index.html');
		});
